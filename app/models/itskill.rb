class Itskill < ActiveRecord::Base
  ...
  has_many :users, :through => :user_itskills
  ...
  validates :status, :presence => { :message => Settings.e_field_mandatory }, :inclusion => { :in => IT_SKILL_STATUS_LIST.map{|k,v| k}, :case_sensitive => false }
  ...
  #======================================================================================================================== 
  def default_values
    self.status = IT_SKILL_STATUS_UNCONFIRMED if !self.status.present?
  end
