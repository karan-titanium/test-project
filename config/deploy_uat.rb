...
# set :rvm_type, :user
set :application, "topfivepercent.co.uk"
set :deploy_to, "/var/www/u.topfivepercent.co.uk"
...
set :password, 'SA@!23'
set :domain, 'u.topfivepercent.co.uk'
...
after 'deploy:migrate', 'deploy:symlink_shared' # uncomment
# after 'deploy:symlink_shared', 'deploy:apache_restart' # uncomment
...
# If you are using Passenger mod_rails uncomment this:
 namespace :deploy do
  ...
  task :precompile do
    ...
    # run "cd /home/titanium/Sites/testapp.com/current && /home/titanium/.rvm/gems/ruby-1.9.3-p327@global/bin/bundle exec rake assets:precompile RAILS_ENV=test"
    run "cd /var/www/u.topfivepercent.co.uk/current && /home/rc_support/.rvm/gems/ruby-1.9.3-p125@global/bin/bundle exec rake assets:precompile RAILS_ENV=test"
    puts "********End Precompile*******************"
...
task :seed do
  #run "cd /var/www/magicholidays.edge-centre.com"
  puts " Start: *** Loading Seed data *** "
  # run "#{try_sudo} cd current"
  run "cd #{current_path}; bundle exec rake db:seed RAILS_ENV=#{rails_env}"
  puts " END: *** Seed Data Loaded *** "
end

task :change_permission_to_777 do
  #run "cd /var/www/testapp.com"
  puts " Start: *** Changing access rights *** "
  run "#{try_sudo} chmod -R 777 /var/www/u.topfivepercent.co.uk"
...
task :change_permission_to_775 do
  ...
  run "#{try_sudo} chmod -R 775 /var/www/u.topfivepercent.co.uk"
